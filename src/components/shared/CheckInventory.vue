<!-- *** if modification is done to moel dialog template for check inventory
update the template inside LastViewedTabItems.vue
-->
<template>
  <div class="check-inv-btn-block">
    <sana-button
...
import ProductHelper from "../../helpers/product-line-helper";
...
export default {
  ...
  methods: {
    ...
    getGlobalInventory() {
      ...
      const productType = ProductHelper.GetProductTypeValueById(
        this.ProductType
      );
      const options = {
        id: this.ProductId,
        type:
          productType && productType.Id === this.ProductType
            ? productType.Title
            : this.ProductType,
        qty: this.Quantity
      };
      if (this.Emit) {
        this.$emit("fetchGlobalInventory", options);
      } else {
