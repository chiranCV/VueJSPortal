<template>
  <div class="cus-info-block">
    <h6>
      {{this.GetSanaText('IOO_CustomerSelection_Customer',
    ...
    <div class="float-right">
      <a class="zfbtn ioo-pt-2" data-toggle="dropdown">
        {{this.GetSanaText('IOO_CustomerSelection_Change',
        'Change')}}
        <span
          class="downArrow"
        ></span>
      ...
      <ul class="dropdown-menu dropdown-menu-right">
        ...
        <li>
          <button
            ...
            type="button"
            v-text="this.GetSanaText('IOO_CustomerSelection_ApplicationOverview',
      ...
      <a
        class="ioo-pt-2"
        v-text="this.GetSanaText('IOO_CustomerSelection_ManageAccount',
        'Manage Account')"
      ></a>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    accountLineClickHandler(item) {
      const cookieJson = sessionManager.getCookieJSON(
        config.Cookiekeys.ShopStatus
      );

      if (cookieJson) {
        cookieJson.CustomerNo = item.CustomerNo;
        cookieJson.SAPCustomerNo = item.SAPCustomerNo;
        cookieJson.SupplierId = item.SupplierId;

        sessionManager.setCookie(config.Cookiekeys.ShopStatus, cookieJson);
      }
