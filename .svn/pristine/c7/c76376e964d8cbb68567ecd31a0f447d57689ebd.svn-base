  ...
  <div>
    ...
    <div>
      <div
        ...
        v-if="!this.IsDataLoading && this.searchCount > 0"
      >
        ...
        <div class="col-lg-8 col-8">
          ...
          <div
            class="view-list-grid-block float-right"
            v-if="this.pagingInfo.ViewModeSwitchEnabled"
          >
            <ul>
              <li>
      ...
      <sana-loading
        ref="loading_popup"
        v-bind:HeaderText="this.GetSanaText('IOO_Popup_Loading','Please wait...')"
        v-bind:Description="this.GetSanaText('IOO_Popup_Desc','Data is loading!')"
        v-bind:ShowPopup="this.IsDataLoading"
      ></sana-loading>
      <div v-if="!this.IsDataLoading">
        ...
        <div
          ...
          v-if="!searchResult.length && !this.searchOptions.query.length"
        >
          <p v-text="this.GetSanaText('IOO_SearchPage_Search','Please search for results.')"></p>
        </div>
      </div>
      <div class="row">
        <div class="col-12">
...
import LoadingPopup from "../shared/ui/LoadingPopup.vue";
...
export default {
  ...
  methods: {
    ...
    searchProducts() {
      ...
      this.fetchSearchResult(this.searchOptions)
        .then(() => {
          const query = {};
          ...
          if (this.pagingInfo.ViewModeSwitchEnabled) {
            if (this.searchOptions.viewMode !== undefined) {
              if (this.searchOptions.viewMode.toLowerCase() === "grid") {
                query.viewMode = "grid";
              }
            }
          } else if (this.pagingInfo.DefaultViewMode.toLowerCase() === "grid") {
            // query.viewMode = "grid";
    ...
    onViewChange(islistview, triggerSearch = true) {
      if (!islistview) {
      ...
      if (triggerSearch) this.searchProducts();
      this.IsDataLoading = false;
    },
    ...
    handleProductSearch(searchKey) {
      this.searchOptions = { query: searchKey, pageno: 1, pagesize: 9 };
