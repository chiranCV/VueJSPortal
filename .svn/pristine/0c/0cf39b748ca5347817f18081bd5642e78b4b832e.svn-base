<template>
  <div>
    ...
    <sana-model-popup ref="print_preview_model_popup" ModalDialogCss="modal-dialog print-popup-Container">
...
export default {
  ...
  computed: {
    PrintPreviewUrl() {
      return `#/Product/${this.$route.params.id}?print=1&type=${
        this.PreSelectedProductType
      }&quantity=${this.PreSelectedQuantity}&date=${
        this.PreSelectedRequestDeliveryDate
      }`;
    }
  },
  methods: {
    ...
    printPage() {
      ...
      printJS({
        ...
        type: "html",
        showModal: true,
        targetStyles: ["modal-body", "product-info"],
        // style: style1
        scanStyles: true
      });
    }
